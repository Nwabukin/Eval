...
interface ReportAnswer {
  ...
  question: {
    text: string;
    weight: number;
    sheet: { name: string; weight: number; minScore: number; maxScore: number };
  };
}
...
interface ReportStats {
  ...
  byDepartment: { name: string; count: number; avgFinal: number }[];
...
export class ReportsService {
  ...
  /**
   * Returns dashboard summary stats with department breakdown
   * and score averages for finalized evaluations.
   * Scoped by role: admins see all, managers see their reports, calibrators see assigned cycles.
   */
  async getDashboard(cycleId?: string, user?: AuthenticatedUser) {
    const where = {
      ...(cycleId ? { cycleId } : {}),
      ...this.buildScopeFilter(user),
    };

    const [statusCounts, totalEmployees, totalDepartments] =
      await Promise.all([
    ...
    for (const entry of statusCounts) {
      if (entry.status === EvaluationStatus.DRAFT) stats.draft = entry._count.id;
      if (entry.status === EvaluationStatus.SUBMITTED_TO_MANAGER) stats.submittedToManager = entry._count.id;
      if (entry.status === EvaluationStatus.SUBMITTED_TO_CALIBRATION) stats.submittedToCalibration = entry._count.id;
      if (entry.status === EvaluationStatus.FINALIZED) stats.finalized = entry._count.id;
    }
  ...
  /**
   * Generates a CSV string of evaluations for a cycle.
   * Proper RFC 4180 escaping for safe spreadsheet import.
   * @throws NotFoundException if the cycle does not exist
   */
  async generateCsvReport(cycleId: string, user?: AuthenticatedUser): Promise<string> {
   ...
   * - Summary sheet (KPIs, department breakdown)
   * - Detailed evaluations sheet (full scores per question with sheet info)
   * @throws NotFoundException if the cycle does not exist
   */
  async generateExcelReport(cycleId: string, user?: AuthenticatedUser): Promise<Buffer> {
  ...
  /**
   * Generates a professionally branded PDF report with:
   * - Branded header on every page with cycle info
   * - KPI summary cards (status counts, averages)
   * - Score distribution analysis (dynamic buckets)
   * - Department breakdown table
   * - Full evaluation detail table with conditional formatting
   * @throws NotFoundException if the cycle does not exist
   */
  async generatePdfReport(cycleId: string, user?: AuthenticatedUser): Promise<Buffer> {
    const cycle = await this.assertCycleExists(cycleId);
    const evaluations = await this.getEvaluationsForReport(cycleId, user);
    const stats = this.computeStats(evaluations);
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      doc.text(
        `Cycle: ${cycle.name}  •  ${this.fmtDate(cycle.startDate)} — ${this.fmtDate(cycle.endDate)}  •  Generated: ${this.fmtDate(new Date())}`,
        PAGE_MARGIN, y,
      );
      y += 28;
      ...
      for (let i = 0; i < totalPages; i++) {
        ...
        doc.text(
          "Performance Evaluation System — Confidential",
          PAGE_MARGIN,
          doc.page.height - 30,
  ...
  private async assertCycleExists(cycleId: string): Promise<CycleInfo> {
  ...
  private async getEvaluationsForReport(cycleId: string, user?: AuthenticatedUser): Promise<ReportEvaluation[]> {
  ...
  /**
   * Builds a Prisma where-clause fragment to restrict evaluations by role.
   * Admins see everything, managers see their direct reports, calibrators see assigned cycles.
   */
  private buildScopeFilter(user?: AuthenticatedUser): Record<string, unknown> {
  ...
  /**
   * Computes weighted statistics across evaluations.
   * Per-question: normalized score = (score / maxScore) * question.weight
   * Score distribution uses dynamic buckets based on the global max score.
   */
  private computeStats(evaluations: ReportEvaluation[]): ReportStats {
    ...
    const deptMap = new Map<string, { count: number; finalSum: number; finalCount: number }>();
    ...
    for (const evaluation of evaluations) {
      ...
      for (const a of evaluation.answers) {
        ...
        if (a.selfScore != null) { selfSum += a.selfScore; selfCount++; }
        if (a.managerScore != null) { mgrSum += a.managerScore; mgrCount++; }
  ...
  private drawPdfHeader(doc: PDFKit.PDFDocument, cycle: CycleInfo, pageW: number): void {
    doc.save();
    ...
    doc.text(
      `${cycle.name}  •  ${STATUS_LABELS[cycle.status] ?? cycle.status}`,
      PAGE_MARGIN, 42,
    );
    doc.restore();
  }
  ...
  private drawKpiCards(
    doc: PDFKit.PDFDocument, startY: number, contentW: number, stats: ReportStats,
  ): number {
    const cards = [
  ...
  private drawScoreAverages(
    ...
    const scores = [
      { label: "Self Assessment", value: stats.avgSelfScore, color: [59, 130, 246] as [number, number, number] },
      { label: "Manager Review", value: stats.avgManagerScore, color: [139, 92, 246] as [number, number, number] },
      { label: "Final Score", value: stats.avgFinalScore, color: [16, 185, 129] as [number, number, number] },
    ];
  ...
  private drawDepartmentBreakdown(
    doc: PDFKit.PDFDocument, startY: number, contentW: number, stats: ReportStats, _pageW: number,
  ): number {
    doc.fillColor(TEXT_PRIMARY).font("Helvetica-Bold").fontSize(12);
  ...
  private drawEvaluationTable(
    ...
    const drawTableHeader = (yPos: number): number => {
      ...
      let x = PAGE_MARGIN + 4;
      doc.text("Employee", x, yPos + 6, { width: colW.name }); x += colW.name;
      doc.text("Dept", x, yPos + 6, { width: colW.dept }); x += colW.dept;
      doc.text("Status", x, yPos + 6, { width: colW.status }); x += colW.status;
      doc.text("Sheet", x, yPos + 6, { width: colW.sheet }); x += colW.sheet;
      doc.text("Question", x, yPos + 6, { width: colW.question }); x += colW.question;
      doc.text("Self", x, yPos + 6, { width: colW.self, align: "center" }); x += colW.self;
      doc.text("Mgr", x, yPos + 6, { width: colW.mgr, align: "center" }); x += colW.mgr;
      doc.text("Final", x, yPos + 6, { width: colW.final, align: "center" }); x += colW.final;
    ...
    for (const evaluation of evaluations) {
      ...
      for (let ai = 0; ai < evaluation.answers.length; ai++) {
        ...
        const drawScore = (score: number | null, w: number) => {
          if (score == null) {
            doc.fillColor(TEXT_SECONDARY).text("—", x, y + 4, { width: w, align: "center" });
          } else {
            const ratio = score / maxS;
            const scoreColor: [number, number, number] =
              ratio >= 0.8 ? [16, 185, 129] : ratio >= 0.6 ? [59, 130, 246] : ratio >= 0.4 ? [245, 158, 11] : [239, 68, 68];
  ...
  // ─── Private: Excel Sheet Builders ──────────────────────

  private buildSummarySheet(workbook: ExcelJS.Workbook, cycle: CycleInfo, stats: ReportStats): void {
  ...
  private buildDetailSheet(workbook: ExcelJS.Workbook, evaluations: ReportEvaluation[]): void {
  ...
  private buildDepartmentSheet(workbook: ExcelJS.Workbook, stats: ReportStats): void {
